...
export default function SolarProjectsModule({ solarProjects, setSolarProjects, isAdmin = false, currentUser: _currentUser = null }) {
  ...
  const handleCreateProject = (e) => {
    e.preventDefault();
    if (!isAdmin) {
      alert('Access Denied: Only Administrator accounts can create solar projects.');
      return;
    }
  ...
  const handleStartEdit = (project) => {
    if (!isAdmin) {
      alert('Access Denied: Only Administrator accounts can edit solar projects.');
      return;
    }
  ...
  const handleUpdateProject = (e) => {
    e.preventDefault();
    if (!isAdmin) {
      alert('Access Denied: Only Administrator accounts can modify solar projects.');
      return;
    }
  ...
  const handleDeleteProject = (id, customer) => {
    if (!isAdmin) {
      alert('Access Denied: Only Administrator accounts can delete solar projects.');
      return;
    }
  ...
  const updateProjectStatus = (id, newStatus) => {
    if (!isAdmin) {
      alert('Access Denied: Only Administrator accounts can change project status.');
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
        ...
        {isAdmin ? (
          ...
          <button
            onClick={() => alert("Access Denied: Creating solar projects is restricted to Administrator accounts.")}
            ...
            title="Project creation restricted to Administrator"
          >
            <Lock className="w-4 h-4 text-amber-600" />
            <span>New Project (Admin Only)</span>
          </button>
        )}
      </div>
          ...
          >
            ...
            <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
              <select
                ...
                title={!isAdmin ? 'Status modification restricted to Administrator' : 'Change Project Status'}
              >
