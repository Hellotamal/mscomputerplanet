...
export default function ClientsModule({ clients, setClients, initialCategory = 'All' }) {
  ...
  const [clientForm, setClientForm] = useState({
    ...
    name: '',
    category: 'AMC',
    leadSource: 'Google Search (Organic/Local)',
    contactPerson: '',
    phone: '',
    email: '',
    address: '',
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredClients.map((client) => {
          ...
          return (
            ...
            >
              <div>
                {/* Card Top: Category, Lead Source & Status */}
          ...
          <div className="bg-white rounded-3xl max-w-2xl w-full p-6 shadow-2xl border border-slate-100 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSaveClient} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1">Lead & Acquisition Channel</label>
                  <select
                    value={clientForm.leadSource || 'Google Search (Organic/Local)'}
                    onChange={(e) => setClientForm({ ...clientForm, leadSource: e.target.value })}
                    className="w-full px-3 py-2 text-xs rounded-xl border border-slate-200 bg-white"
                  >
                    <option value="Google Search (Organic/Local)">Google Search (Organic/Local 3-Pack)</option>
                    <option value="IndiaMART B2B Lead">IndiaMART TrustSEAL / Inquiry</option>
                    <option value="Justdial Directory">Justdial 5-Star Listing</option>
                    <option value="TradeIndia / TradeMart">TradeIndia / TradeMart B2B</option>
                    <option value="GeM Govt Portal">Government e-Marketplace (GeM)</option>
                    <option value="Client Referral / Word of Mouth">Client Referral / Word of Mouth</option>
                    <option value="Omnis Trades Channel">Omnis Trades Solar Channel Referral</option>
                    <option value="Direct Walk-in / Field Survey">Direct Walk-in / Field Survey</option>
                  </select>
                </div>
